...
# Adjust paths to point to the actual model directory at the backend/models directory
...
def predict(features_dict: dict) -> tuple:
    ...
    feature_names = [
        "has_rekey_check", "has_close_check", "has_receiver_check", 
        "num_txn", "num_and", "num_or", "contract_length",
        "txn_per_length", "logic_density", "security_checks", "txn_logic_ratio"
    ]
    ...
    # Ensure it's compatible whether inverse_transform works with list or scalar depends on version, usually list
    prediction_label = encoder.inverse_transform([prediction_num])[0]
    
    # Some older encodings output string, but in case they don't, we map them directly if needed
    # The requirement says label must be SAFE / SUSPICIOUS / RISKY
    
    if hasattr(prediction_num, "item"):
        prediction_num = prediction_num.item()
        
    return prediction_num, str(prediction_label)
